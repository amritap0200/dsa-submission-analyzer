...
def generate_cpg(file_path: Path, out_dir: Path, timeout_sec=120):
    ...
    # isolate the file so the frontend parses only this submission
    ...
    try:
        ...
        if result.returncode != 0 or not cpg_path.exists():
            return {
                "status": "failed",
                "language": language,
                "stderr": result.stderr[-2000:],  # keep log size sane
            }
...
def run_batch(dataset_dir: str, output_dir: str):
    ...
    manifest = {"run_timestamp": datetime.now().isoformat(), "results": {}}
    ...
    for f in files:
        ...
        if result["status"] == "success":
            ...
            log.warning(f"  SKIPPED: {student_id} - {result['reason']}")
        else:
            log.error(f"  FAILED: {student_id} - {result.get('reason', result.get('stderr', 'unknown error'))}")
    ...
    skipped = sum(1 for r in manifest["results"].values() if r["status"] == "skipped")
