...
class ASTNode:
	def __init__(self, node_type, label = None, parent = None, text = None): #only give text argument if node is a leaf
		...
		self.children = []
		self.text = text
		self.height = 0 #will be changed when the tree is put through the add_heights() function
	...
	def calc_structure_hash(self):
		child_hashes = tuple(child.calc_structure_hash() for child in self.children) 
		return hash((self.type, self.label, child_hashes))
	
	def to_dict(self):
		obj = {"type": self.type, "label": self.label, "text": self.text, "height": self.height}
		if self.children:
			children = []
			for child in self.children:
				children.append(child.to_dict())
			obj[f"children ({len(children)})"] = children
		return obj

	def to_dict_no_children(self):
		return {"type": self.type, "label": self.label, "text": self.text, "height": self.height, "children_count": len(self.children)}
...
def add_heights(cur):
	if not cur.children:
		...
		for child in cur.children:
			heights.append(add_heights(child))
		cur.height = max(heights) + 1
		return cur.height

def convert_to_ast(TSCursor, language):
	tree = None
	if language.lower() == "c": 
		tree = convert(TSCursor, None, C_Language())[0]
	elif language.lower() == "python":
		tree = convert(TSCursor, None, Python_Language())[0]
	else:
