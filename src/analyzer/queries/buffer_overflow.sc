...
@main def exec(cpgPath: String) = {
  ...
  cpg.call.name("<operator>.indirectIndexAccess").foreach { access =>
    ...
    indexArg.foreach { idx =>
      val isIdentifierIndex = idx.label == "IDENTIFIER"
      if (isIdentifierIndex) {
        ...
        if (!guarded) {
          results += Map(
            ...
            "node_id" -> access.id,
            "description" -> s"Array access at line ${access.lineNumber.getOrElse(-1)} indexed by a variable with no reachable bounds check"
          )
        }
      }
    }
